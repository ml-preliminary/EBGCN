...
# 加载Weibo数据集
def main(obj):
    treePath = os.path.join(cwd, "data/" + obj + "/" + "weibotree.txt")
    ...
    labelPath = os.path.join(cwd, "data/" + obj + "/" + "weibo_id_label.txt")
    ...
    with open(labelPath, 'r') as f:
        ...
        num_samples_to_load = int(num_samples * 0.15)  # Load only 15% of the data

    # Load only 5% of the lines
    for i, line in enumerate(lines[:num_samples_to_load]):
        line = line.rstrip()
    ...
    # 将ID和标签保存到新的文本文件
    new_labels_path = os.path.join(cwd, 'data', obj, 'new_id_labels.txt')
    with open(new_labels_path, 'w') as label_file:
        for eid, label in labelDic.items():
            label_file.write(f"{eid} {label}\n")
